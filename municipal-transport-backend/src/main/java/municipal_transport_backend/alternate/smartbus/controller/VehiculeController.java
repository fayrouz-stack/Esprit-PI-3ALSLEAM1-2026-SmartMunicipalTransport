...
import municipal_transport_backend.alternate.smartbus.repository.PositionHistoryRepository;
...
public class VehiculeController {
    ...
    public ResponseEntity<?> updatePosition(
        ...
        positionHistoryRepository.save(new PositionHistory(id, lat, lon));

        // Broadcaster via WebSocket natif à tous les clients connectés
        Map<String, Object> update = new HashMap<>();
        update.put("id",             id);
        update.put("latitude",       lat);
        update.put("longitude",      lon);
        update.put("marque",         v.getMarque());
        update.put("modele",         v.getModele());
        update.put("matriculeFourni",v.getMatriculeFourni());
        update.put("etat",           v.getEtat());
        update.put("timestamp",      System.currentTimeMillis());
        positionWebSocketHandler.broadcast(update);

        return ResponseEntity.ok(Map.of(
                "id", id,
                "latitude", lat,
                "longitude", lon,
                "message", "Position mise à jour et diffusée"
        ));
    }

    /**
     * Retourne les 50 dernières positions GPS d'un véhicule (du plus récent au plus ancien).
     * GET /api/vehicules/{id}/position-history
     */
    @GetMapping("/{id}/position-history")
    public ResponseEntity<List<PositionHistory>> getPositionHistory(@PathVariable Long id) {
        return ResponseEntity.ok(
            positionHistoryRepository.findTop50ByVehiculeIdOrderByRecordedAtDesc(id)
        );
    }
