-- ============================================================
--  SCRIPT DE PEUPLEMENT — Transport Municipal
--  Ordre : TRUNCATE → ligne, station, horaire, vehicule,
--          chauffeur → voyage → ticket
-- ============================================================
...
TRUNCATE TABLE ligne     RESTART IDENTITY CASCADE;
TRUNCATE TABLE vehicule  RESTART IDENTITY CASCADE;
TRUNCATE TABLE chauffeur RESTART IDENTITY CASCADE;

SET session_replication_role = DEFAULT;

-- ============================================================
--  1. LIGNES (10 lignes inter-villes tunisiennes)
-- ============================================================
...
-- ============================================================
--  2. STATIONS (15 stations principales)
-- ============================================================
...
-- ============================================================
--  3. HORAIRES (20 créneaux horaires — juin/juillet 2026)
-- ============================================================
...
-- ============================================================
--  4. VÉHICULES (12 véhicules — flotte variée)
-- ============================================================
...
-- ============================================================
--  5. CHAUFFEURS (12 chauffeurs professionnels)
-- ============================================================
...
-- ============================================================
--  6. VOYAGES (15 voyages avec liaisons réalistes)
-- ============================================================
INSERT INTO voyage (
  date_voyage, nombre_places_disponible, prix,
  ligne_id, horaire_id, vehicule_id, matricule_cond
) VALUES
  ('2026-06-01',  20, 15.0,  1,  2,  1, 'BUS-01'),
  ('2026-06-01',  45, 35.0,  2,  4,  2, 'BUS-02'),
  ('2026-06-01',  12, 10.0,  3,  1,  4, 'BUS-03'),
  ('2026-06-02',  20, 15.0,  1,  5,  1, 'BUS-01'),
  ('2026-06-02',  50, 40.0,  2,  7,  6, 'BUS-04'),
  ('2026-06-02',  30, 20.0,  4,  6,  8, 'BUS-05'),
  ('2026-06-03',  45, 35.0,  2,  8, 11, 'BUS-06'),
  ('2026-06-03',  18, 12.0,  5,  9,  4, 'BUS-07'),
  ('2026-06-05',  40, 30.0,  7, 12,  7, 'BUS-08'),
  ('2026-06-05',  50, 45.0,  8, 11, 11, 'BUS-09'),
  ('2026-06-10',  20, 15.0,  1, 12,  1, 'BUS-10'),
  ('2026-06-10',  35, 25.0,  6, 13, 10, 'BUS-11'),
  ('2026-06-15',  45, 35.0,  2, 14,  2, 'BUS-12'),
  ('2026-06-20',  20, 18.0,  9, 16,  4, 'BUS-03'),
  ('2026-07-01',  50, 50.0, 10, 18,  8, 'BUS-06');

-- ============================================================
--  7. TICKETS (20 réservations)
-- ============================================================
INSERT INTO ticket (
...
-- Vérification
SELECT 'chauffeur' AS table_name, COUNT(*) FROM chauffeur
UNION ALL SELECT 'vehicule',  COUNT(*) FROM vehicule
UNION ALL SELECT 'ligne',     COUNT(*) FROM ligne
UNION ALL SELECT 'horaire',   COUNT(*) FROM horaire
UNION ALL SELECT 'station',   COUNT(*) FROM station
UNION ALL SELECT 'voyage',    COUNT(*) FROM voyage
